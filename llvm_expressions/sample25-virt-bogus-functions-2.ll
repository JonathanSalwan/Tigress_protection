; ModuleID = ""
target triple = "x86_64-pc-linux-gnu"
target datalayout = ""

define i64 @"SECRET"(i64 %"SymVar_0") nounwind
{
.3:
  %".4" = sext i64 18014398509482166540 to i128
  %".5" = zext i64 %"SymVar_0" to i128
  %".6" = zext i64 0 to i128
  %".7" = shl i128 %".6", 64
  %".8" = or i128 %".5", %".7"
  %".9" = zext i64 6 to i128
  %".10" = udiv i128 %".8", %".9"
  %".11" = trunc i128 %".10" to i64
  %".12" = sext i64 %".11" to i128
  %".13" = mul i128 %".4", %".12"
  %".14" = trunc i128 %".13" to i64
  %".15" = sext i64 %".14" to i128
  %".16" = zext i8 51 to i64
  %".17" = and i64 %".16", 63
  %".18" = lshr i64 %"SymVar_0", %".17"
  %".19" = zext i8 13 to i64
  %".20" = and i64 %".19", 63
  %".21" = shl i64 %"SymVar_0", %".20"
  %".22" = or i64 %".18", %".21"
  %".23" = sext i64 %".22" to i128
  %".24" = mul i128 %".15", %".23"
  %".25" = trunc i128 %".24" to i64
  %".26" = sext i64 81897458 to i128
  %".27" = add i64 %".14", %"SymVar_0"
  %".28" = sub i64 %".22", 785763710
  %".29" = sub i64 686160936, %".28"
  %".30" = zext i8 2 to i64
  %".31" = and i64 %".30", 63
  %".32" = lshr i64 %".29", %".31"
  %".33" = and i64 7, %".32"
  %".34" = or i64 1, %".33"
  %".35" = trunc i64 %".34" to i32
  %".36" = zext i32 %".35" to i64
  %".37" = trunc i64 %".36" to i8
  %".38" = zext i8 %".37" to i64
  %".39" = and i64 %".38", 63
  %".40" = lshr i64 %".27", %".39"
  %".41" = zext i8 1 to i64
  %".42" = and i64 %".41", 63
  %".43" = lshr i64 %".40", %".42"
  %".44" = and i64 7, %".43"
  %".45" = or i64 1, %".44"
  %".46" = trunc i64 %".45" to i32
  %".47" = zext i32 %".46" to i64
  %".48" = trunc i64 %".47" to i8
  %".49" = zext i8 %".48" to i64
  %".50" = and i64 %".49", 63
  %".51" = lshr i64 %".40", %".50"
  %".52" = trunc i64 %".51" to i8
  %".53" = zext i8 %".52" to i32
  %".54" = lshr i64 %".51", 8
  %".55" = trunc i64 %".54" to i8
  %".56" = zext i8 %".55" to i32
  %".57" = shl i32 %".56", 8
  %".58" = or i32 %".53", %".57"
  %".59" = lshr i64 %".51", 16
  %".60" = trunc i64 %".59" to i8
  %".61" = zext i8 %".60" to i32
  %".62" = shl i32 %".61", 16
  %".63" = or i32 %".58", %".62"
  %".64" = lshr i64 %".51", 24
  %".65" = trunc i64 %".64" to i8
  %".66" = zext i8 %".65" to i32
  %".67" = shl i32 %".66", 24
  %".68" = or i32 %".63", %".67"
  %".69" = zext i32 %".68" to i64
  %".70" = trunc i64 %".69" to i32
  %".71" = zext i32 %".70" to i64
  %".72" = trunc i64 %".71" to i32
  %".73" = zext i32 %".72" to i64
  %".74" = trunc i64 %".73" to i32
  %".75" = zext i32 %".74" to i64
  %".76" = trunc i64 %".75" to i32
  %".77" = zext i32 %".76" to i64
  %".78" = trunc i64 %".77" to i32
  %".79" = zext i32 %".78" to i64
  %".80" = trunc i64 %".79" to i32
  %".81" = trunc i32 %".80" to i8
  %".82" = zext i8 %".81" to i64
  %".83" = trunc i64 %".79" to i32
  %".84" = lshr i32 %".83", 8
  %".85" = trunc i32 %".84" to i8
  %".86" = zext i8 %".85" to i64
  %".87" = shl i64 %".86", 8
  %".88" = or i64 %".82", %".87"
  %".89" = trunc i64 %".79" to i32
  %".90" = lshr i32 %".89", 16
  %".91" = trunc i32 %".90" to i8
  %".92" = zext i8 %".91" to i64
  %".93" = shl i64 %".92", 16
  %".94" = or i64 %".88", %".93"
  %".95" = trunc i64 %".79" to i32
  %".96" = lshr i32 %".95", 24
  %".97" = trunc i32 %".96" to i8
  %".98" = zext i8 %".97" to i64
  %".99" = shl i64 %".98", 24
  %".100" = or i64 %".94", %".99"
  %".101" = lshr i64 %".51", 32
  %".102" = trunc i64 %".101" to i8
  %".103" = zext i8 %".102" to i32
  %".104" = lshr i64 %".51", 40
  %".105" = trunc i64 %".104" to i8
  %".106" = zext i8 %".105" to i32
  %".107" = shl i32 %".106", 8
  %".108" = or i32 %".103", %".107"
  %".109" = lshr i64 %".51", 48
  %".110" = trunc i64 %".109" to i8
  %".111" = zext i8 %".110" to i32
  %".112" = shl i32 %".111", 16
  %".113" = or i32 %".108", %".112"
  %".114" = lshr i64 %".51", 56
  %".115" = trunc i64 %".114" to i8
  %".116" = zext i8 %".115" to i32
  %".117" = shl i32 %".116", 24
  %".118" = or i32 %".113", %".117"
  %".119" = zext i32 %".118" to i64
  %".120" = trunc i64 %".119" to i32
  %".121" = zext i32 %".120" to i64
  %".122" = trunc i64 %".121" to i32
  %".123" = zext i32 %".122" to i64
  %".124" = trunc i64 %".123" to i32
  %".125" = zext i32 %".124" to i64
  %".126" = trunc i64 %".125" to i32
  %".127" = zext i32 %".126" to i64
  %".128" = trunc i64 %".127" to i32
  %".129" = zext i32 %".128" to i64
  %".130" = trunc i64 %".129" to i32
  %".131" = trunc i32 %".130" to i8
  %".132" = zext i8 %".131" to i64
  %".133" = shl i64 %".132", 32
  %".134" = or i64 %".100", %".133"
  %".135" = trunc i64 %".129" to i32
  %".136" = lshr i32 %".135", 8
  %".137" = trunc i32 %".136" to i8
  %".138" = zext i8 %".137" to i64
  %".139" = shl i64 %".138", 40
  %".140" = or i64 %".134", %".139"
  %".141" = trunc i64 %".129" to i32
  %".142" = lshr i32 %".141", 16
  %".143" = trunc i32 %".142" to i8
  %".144" = zext i8 %".143" to i64
  %".145" = shl i64 %".144", 48
  %".146" = or i64 %".140", %".145"
  %".147" = trunc i64 %".129" to i32
  %".148" = lshr i32 %".147", 24
  %".149" = trunc i32 %".148" to i8
  %".150" = zext i8 %".149" to i64
  %".151" = shl i64 %".150", 56
  %".152" = or i64 %".146", %".151"
  %".153" = sext i64 %".152" to i128
  %".154" = mul i128 %".26", %".153"
  %".155" = trunc i128 %".154" to i64
  %".156" = and i64 7, %".22"
  %".157" = zext i8 2 to i64
  %".158" = and i64 %".157", 63
  %".159" = shl i64 %".156", %".158"
  %".160" = or i64 %".14", %".22"
  %".161" = add i64 %".160", %"SymVar_0"
  %".162" = or i64 %".159", %".161"
  %".163" = and i64 7, %".162"
  %".164" = zext i8 2 to i64
  %".165" = and i64 %".164", 63
  %".166" = shl i64 %".163", %".165"
  %".167" = or i64 %".166", %".162"
  %".168" = xor i64 %".155", %".167"
  %".169" = and i64 15, %".168"
  %".170" = or i64 1, %".169"
  %".171" = sub i64 64, %".170"
  %".172" = trunc i64 %".171" to i32
  %".173" = zext i32 %".172" to i64
  %".174" = trunc i64 %".173" to i8
  %".175" = zext i8 %".174" to i64
  %".176" = and i64 %".175", 63
  %".177" = lshr i64 %".25", %".176"
  %".178" = sext i64 %".14" to i128
  %".179" = sext i64 %".22" to i128
  %".180" = mul i128 %".178", %".179"
  %".181" = trunc i128 %".180" to i64
  %".182" = sext i64 81897458 to i128
  %".183" = zext i8 %".81" to i64
  %".184" = zext i8 %".85" to i64
  %".185" = shl i64 %".184", 8
  %".186" = or i64 %".183", %".185"
  %".187" = zext i8 %".91" to i64
  %".188" = shl i64 %".187", 16
  %".189" = or i64 %".186", %".188"
  %".190" = zext i8 %".97" to i64
  %".191" = shl i64 %".190", 24
  %".192" = or i64 %".189", %".191"
  %".193" = zext i8 %".131" to i64
  %".194" = shl i64 %".193", 32
  %".195" = or i64 %".192", %".194"
  %".196" = zext i8 %".137" to i64
  %".197" = shl i64 %".196", 40
  %".198" = or i64 %".195", %".197"
  %".199" = zext i8 %".143" to i64
  %".200" = shl i64 %".199", 48
  %".201" = or i64 %".198", %".200"
  %".202" = zext i8 %".149" to i64
  %".203" = shl i64 %".202", 56
  %".204" = or i64 %".201", %".203"
  %".205" = sext i64 %".204" to i128
  %".206" = mul i128 %".182", %".205"
  %".207" = trunc i128 %".206" to i64
  %".208" = xor i64 %".207", %".167"
  %".209" = and i64 15, %".208"
  %".210" = or i64 1, %".209"
  %".211" = trunc i64 %".210" to i32
  %".212" = zext i32 %".211" to i64
  %".213" = trunc i64 %".212" to i8
  %".214" = zext i8 %".213" to i64
  %".215" = and i64 %".214", 63
  %".216" = shl i64 %".181", %".215"
  %".217" = or i64 %".177", %".216"
  ret i64 %".217"
}
